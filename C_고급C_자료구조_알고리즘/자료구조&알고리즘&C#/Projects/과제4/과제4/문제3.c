...
TreeNode* sibling(TreeNode *z) {
	if (z->parent->lChild == z)return z->parent->rChild;//z의 부모노드의 lChild 가 z면 rChild가 형제
	else return z->parent->lChild;//반대면 lChild가 형제
}//z의 형제 노드 찾는 함수
...
TreeNode* reduceExternal(TreeNode *z) {
	...
	if (p == root) {
		root = zs;
		zs->parent = NULL;
	}//p가 root 면 root를 zs로 대체한다.
	else {
		g = p->parent;
		zs->parent = g;
		if (p == g->lChild) g->lChild = zs;
		else g->rChild = zs;
	}//z의 형제노드를 삭제하려는 노드의 부모노드에 이어주고 g 노드의 왼쪽인지 오른쪽인지 판단하여 이어준다.

	free(z);
	free(p);
	//z와 z의 부모노드를 삭제하기 위해 동적할당 해제를 해준다.
...
void searchAndRepairAfterRemoval(TreeNode *w) {
	...
	while (heightUpdateAndBalanceCheck(z)) {
		if (root == z) return;
		z = z->parent;
	}//계속 위로 올라가면서 불균형 노드를 찾는다. 만약 z가 root면 return

	if (z->lChild->height > z->rChild->height) y = z->lChild;
	else y = z->rChild;
	//z의 자식중 높이가 높은 것을 y라 한다.

	if (y->lChild->height > y->rChild->height) x = y->lChild;
	else if (y->lChild->height < y->rChild->height) x = y->rChild;
	else if (z->lChild == y) x = y->lChild;
	else x = y->rChild;
	//자식의 높이를 비교해서 x를 결정한다.

	b = restructure(x);// 개조
	searchAndRepairAfterRemoval(b); // 전체적인 균형을 맞추기 위해서 재귀를 한다.
	return;
}//높이 균형 회복 함수

int removeElement(int k) {
	TreeNode *w = treeSearch(root, k);//삭제 노드 찾기
	TreeNode *z,*zs,*y;

	if (isExternal(w)) {
		printf("X\n");
		return;
	} //삭제하려는 노드가 없으면 X를 프린트하고 return
	...
	if (isExternal(z)) zs = reduceExternal(z);
		...
		w->key = y->key;
		zs = reduceExternal(z);
	}//w의 자식 중 하나가 외부노드인경우와 아닌경우를 선택해서 삭제를 진행한다.
